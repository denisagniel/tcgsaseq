...
col_data <- DataFrame(model.matrix(~group))
rownames(col_data) <- colnames(London[,-group_London_LTBI])
rse <- SummarizedExperiment(assays = London[,-group_London_LTBI], 
                            colData = col_data)
res_dearseq <- dearseq::dear_seq(object = rse, 
...
data('baduel_5gs')
rse2 <- SummarizedExperiment(assay = log2(expr_norm_corr+1), 
                             colData = design)
KAvsTBG <- dearseq::dgsa_seq(object = rse2,
...
Cold <- dearseq::dgsa_seq(object = rse2,
