...
```{r baduel, eval=TRUE, echo=TRUE}
library(dearseq)
library(SummarizedExperiment)
data('baduel_5gs')
se2 <- SummarizedExperiment(assay = log2(expr_norm_corr+1), 
                            colData = design)
genes0var_ind <- which(rowVars(expr_norm_corr)==0)
KAvsTBG <- dearseq::dgsa_seq(object = se2[-genes0var_ind, ],
...
Cold <- dearseq::dgsa_seq(object = se2[-genes0var_ind, ],
