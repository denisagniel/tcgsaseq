...
```{r LR_ST_echo, eval=FALSE, echo=TRUE}
library(dearseq)
library(SummarizedExperiment)
# Remember the Latent TB samples are excluded 
col_data <- data.frame(model.matrix(~group))
rownames(col_data) <- colnames(London[,-group_London_LTBI])
se <- SummarizedExperiment(assays = London[,-group_London_LTBI], 
                            colData = col_data)
res_dearseq <- dearseq::dear_seq(object = se, 
                                 covariates = "X.Intercept.",
                                 variables2test = "group",
                                 which_test='asymptotic', doPlot = FALSE,
                                 preprocessed=TRUE)
```
```{r LR_ST_eval, eval=TRUE, echo=FALSE}
library(SummarizedExperiment)
col_data <- data.frame(model.matrix(~group))
rownames(col_data) <- colnames(London[,-group_London_LTBI])
se <- SummarizedExperiment::SummarizedExperiment(assays = London[,-group_London_LTBI], 
                            colData = col_data)
res_dearseq <- dearseq::dear_seq(object = se, 
                                 covariates = "X.Intercept.",
                                 variables2test = "group",
                                 which_test='asymptotic', doPlot = FALSE,
                                 preprocessed=TRUE, parallel_comp = FALSE)
```
...
```{r baduel_echo, eval=FALSE, echo=TRUE}
data('baduel_5gs')
se2 <- SummarizedExperiment(assay = log2(expr_norm_corr+1), 
                             colData = design)
KAvsTBG <- dearseq::dgsa_seq(object = se2,
                             covariates=c('Intercept', 'Vernalized', 'AgeWeeks', 'Vernalized_Population', 'AgeWeeks_Population'),
                             variables2test = 'PopulationKA',
                             genesets=baduel_gmt$genesets[c(3,5)],
                             which_test = 'permutation', which_weights = 'loclin',
                             n_perm=1000, preprocessed = TRUE, doPlot = FALSE, 
                             verbose = FALSE)
KAvsTBG$pvals

Cold <- dearseq::dgsa_seq(object = se2,
                          covariates = c('Intercept', 'AgeWeeks', 'PopulationKA', 'AgeWeeks_Population'),
...
```{r baduel_eval, eval=TRUE, echo=FALSE}
library(dearseq)
library(SummarizedExperiment)
data('baduel_5gs')

se2 <- SummarizedExperiment::SummarizedExperiment(assay = log2(expr_norm_corr+1), 
                             colData = design)
KAvsTBG <- dearseq::dgsa_seq(object = se2,
                    covariates=c('Intercept', 'Vernalized', 'AgeWeeks', 'Vernalized_Population', 'AgeWeeks_Population'),
                    variables2test = 'PopulationKA',
